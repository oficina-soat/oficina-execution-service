...
public class BuscarExecucaoUseCase {
    private final ExecucaoGateway execucaoGateway;

    public BuscarExecucaoUseCase(ExecucaoGateway execucaoGateway) {
        this.execucaoGateway = execucaoGateway;
    }

    public CompletableFuture<Execucao> executar(UUID execucaoId) {
        return execucaoGateway.buscarExecucao(execucaoId);
    }
}
